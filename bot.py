...
# -*- coding: utf-8 -*-
"""
TELEGRAM BOT - КОНТРОЛЬ ПЛАНА ПРОДАЖ v4.4 (восстановление, исправления, allow_reentry)
"""
...
def set_extended_report(year: int, month: int, data: dict):
    """Обновляет или создаёт запись расширенного отчёта."""
...
def main():
    ...
    app.add_handler(ConversationHandler(
        ...
        fallbacks=[CommandHandler("cancel", cancel)],
        allow_reentry=True   # ← чтобы кнопка не зависала
    ))
    app.add_handler(ConversationHandler(
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            ...
            EXT_TPROF: [MessageHandler(filters.TEXT & ~filters.COMMAND, e_tprof)],
        },
        fallbacks=[CommandHandler("cancel", cancel)]
    ))
    app.add_handler(ConversationHandler(
    ...
    print("🤖 Бот v4.4 (восстановленный) запущен!")
